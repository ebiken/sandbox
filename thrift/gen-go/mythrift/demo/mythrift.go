...
func (p *MyThriftClient) sendCallBack(callTime int64, name string, paramMap map[string]string)(err error) {
...
func (p *MyThriftClient) recvCallBack() (value []string, err error) {
...
// Parameters:
//  - NewArticle_
func (p *MyThriftClient) Put(newArticle *Article) (err error) {
  if err = p.sendPut(newArticle); err != nil { return }
  return p.recvPut()
}

func (p *MyThriftClient) sendPut(newArticle *Article)(err error) {
  oprot := p.OutputProtocol
  if oprot == nil {
    oprot = p.ProtocolFactory.GetProtocol(p.Transport)
    p.OutputProtocol = oprot
...
func (p *MyThriftClient) recvPut() (err error) {
  iprot := p.InputProtocol
  if iprot == nil {
    iprot = p.ProtocolFactory.GetProtocol(p.Transport)
    p.InputProtocol = iprot
...
func (p *myThriftProcessorCallBack) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
  args := MyThriftCallBackArgs{}
...
func (p *myThriftProcessorPut) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
  args := MyThriftPutArgs{}
  if err = args.Read(iprot); err != nil {
    iprot.ReadMessageEnd()
    x := thrift.NewTApplicationException(thrift.PROTOCOL_ERROR, err.Error())
